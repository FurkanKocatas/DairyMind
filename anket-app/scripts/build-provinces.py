"""Generate data/turkey-provinces.json — all 81 il + districts."""
import json
from pathlib import Path

# 81 il + ilçe (district) — major districts per il, covers >90% of population
# Source: T.C. Içişleri Bakanlığı resmi liste
PROVINCES = {
...
def main():
    out = Path(__file__).resolve().parents[1] / "data" / "turkey-provinces.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    data = {
        "provinces": [
            {"name": il, "districts": districts}
            for il, districts in sorted(PROVINCES.items(), key=lambda x: x[0])
        ]
    }
    out.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {out}  provinces={len(PROVINCES)}  districts={sum(len(d) for d in PROVINCES.values())}")
